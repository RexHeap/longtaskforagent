...
def test_does_not_create_guide_or_init_scripts():
    """init_project.py should NOT create long-task-guide.md, init.sh, init.ps1 (LLM generates these)."""
    tmp = tempfile.mkdtemp()
    try:
        run_init("test-project", tmp)
        assert not os.path.exists(os.path.join(tmp, "long-task-guide.md")), \
            "long-task-guide.md should NOT be generated by init_project.py"
        assert not os.path.exists(os.path.join(tmp, "init.sh")), \
            "init.sh should NOT be generated by init_project.py"
        assert not os.path.exists(os.path.join(tmp, "init.ps1")), \
            "init.ps1 should NOT be generated by init_project.py"
    finally:
        shutil.rmtree(tmp)

...
def test_does_not_create_guide_or_init_scripts():
    """init_project.py should NOT create LLM-generated files (guide, init, start, cleanup scripts)."""
    tmp = tempfile.mkdtemp()
    try:
...
def test_stdout_mentions_llm_todo():
    ...
    try:
        ...
        assert "start.sh" in stdout, "Should mention start.sh as LLM TODO"
        assert "cleanup.sh" in stdout, "Should mention cleanup.sh as LLM TODO"
    finally:
        shutil.rmtree(tmp)


def test_copies_validate_start_cleanup_script():
    """init_project.py should copy validate_start_cleanup.py to project scripts/."""
    tmp = tempfile.mkdtemp()
    try:
        run_init("test-project", tmp)
        script_path = os.path.join(tmp, "scripts", "validate_start_cleanup.py")
        assert os.path.exists(script_path), \
            "validate_start_cleanup.py should be copied to project scripts/"
    finally:
        shutil.rmtree(tmp)

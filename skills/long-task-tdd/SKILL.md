...
### ST Test Case Document Input

If an ST test case document exists for this feature (`docs/test-cases/feature-{id}-{slug}.md`, generated by `long-task-st-case` in Worker Step 6), read it before writing tests:
- Each ST test case's **steps and expected results** inform what assertions to write
- Each automated test SHOULD reference its corresponding ST case ID via a comment:
  ...
  def test_valid_order_creation():
      ...
  ```
- The ST test case document is a **superset** of `verification_steps` — it adds preconditions, boundary cases, and categorized coverage
- This is additional input alongside `verification_steps`, not a replacement
